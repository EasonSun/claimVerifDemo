...
class Classifier(object):
	...
	def __init__(self, task, logPath, experimentPath, modelPath, X=None, y=None):
	...
	def _initRF(self):
		if os.path.isfile(self.modelPath):
			...
			return joblib.load(self.modelPath)
		else:
			if self.task == 'stance':
	...
	def paramSearch(self, n_fold=5, sampleWeight=None):
		...
		from sklearn.externals import joblib
		joblib.dump(self.rf, self.experimentPath+self.task+'/rf.pkl')
		logFile.close()
	...
	# if you don't wanna search params and fit a model right away
	def train():
